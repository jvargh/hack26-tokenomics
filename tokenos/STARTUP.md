...
### Option 2: Single-Line PowerShell Command
```powershell
Start-Process -FilePath "services\tokenos-api\.venv\Scripts\python.exe" -ArgumentList "-m uvicorn tokenos_api.app:app --host 127.0.0.1 --port 8000" -WorkingDirectory "services\tokenos-api"; Start-Process -FilePath "cmd.exe" -ArgumentList "/c npm run dev" -WorkingDirectory "apps\web"
```
...
2. Re-run `npm run dev` in `apps\web`.
